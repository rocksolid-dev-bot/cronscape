...
| Concept | Rule |
|---|---|
| Collision window | Set in the UI — **Collision window (s)**, default **60**, range **0–3600**. An occurrence from a crontab line other than the group's own joins the group when it lands within that many seconds of the group's anchor — the window is anchor-relative, not a simple pairwise distance between any two occurrences; one line firing twice in its own window is not a collision. **0** is legal and means same-instant-only. |
| Collision grouping | Anchored, not chained: the earliest ungrouped occurrence anchors a group, and every later occurrence within the window of that anchor joins it. Bounds every group's spread to at most one window — chaining would let a train of jobs 50s apart each collapse into one group spanning far more than the window. Consequence, measured rather than assumed: a pair of occurrences that would satisfy the window rule on its own is not always its own group, when one member has already been consumed by an earlier anchor. Worked case (window 60s): occurrences at `+0s` (line 1), `+55s` (line 2), `+110s` (line 1) return **one** group anchored at `+0s` with members `+0s`/`+55s` — `+110s` is 110s from that anchor, outside the window, so it starts nothing, and the `+55s`/`+110s` pair (55s apart, inside the window) is never reported, because `+55s` has already been consumed by the first group. That same pair alone, with no `+0s` occurrence to consume it, **is** reported as its own group. Pinned in `test/collisions.test.ts`. |
